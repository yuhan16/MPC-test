function [matrix_ineq, solver_setting] = FormulateSub(A, B, C, D, dlim, Q, Qf, R, ...
...
for i = 1: N
%     if i == 1
%         Amat((i-1)*dimA+1: i*dimA, :) = A;
%     else
%         Amat((i-1)*dimA+1: i*dimA, :) = Amat((i-2)*dimA+1: (i-1)*dimA,:) * A;
%     end
    %Amat((i-1)*dimA+1: i*dimA, :) = A^i;
...
%Smat = sparse([Bmat'*q*Bmat+r Bmat'*q*Cmat zeros(N*dimB,N*dimC);Cmat'*q*Bmat Cmat'*q*Cmat...
%    zeros(N*dimC,N*dimC);zeros(N*dimC,N*(dimB+2*dimC))]);
...
matrix_ineq{14} = Gmat; matrix_ineq{15} = Glim1; matrix_ineq{16} = Glim2;
