function [bigMat, solverConfig] = FormulateRelax(sysdynm, sysparam)
...
bigMat.Dmat = sparse([kron(eye(N), D) zeros(N*dimC, N*(dimB+dimC))]);
bigMat.Dlim = kron(ones(N, 1), dlim);

% Emat * X <= Elim -> Phi(q(k+1)) <= M(1-z(k)) -> Dx(k+1)+dlim <= M(1-z) 
bigMat.Emat = sparse([kron(eye(N), D) zeros(N*dimC, N*dimB) ...
               kron(eye(N), bigM*eye(dimC))]);
bigMat.Elim = kron(ones(N, 1), [bigM; bigM]) - bigMat.Dlim;

% Smat -> J = X' * Smat * X
q = blkdiag(kron(eye(N-1), Q), Qf);
r = kron(eye(N), R);
S = blkdiag(q, r);
bigMat.Smat = sparse([S zeros(N*(dimA+dimB), N*dimC); ...
            zeros(N*dimC, N*(dimA+dimB)) zeros(N*dimC)]);
bigMat.cmat = sparse(zeros(N*(dimA+dimB+dimC), dimA));
bigMat.dmat = sparse(-[q zeros(N*dimA, N*(dimB+dimC))]');
bigMat.Q = Q;
bigMat.q = q;
...
% solver settings
solverConfig.senselst = [repmat('=', 1, N*dimA) repmat('<', 1, 2*N*dimC)];
solverConfig.vtypelst = repmat('C', 1, N*(dimA+dimB+dimC));
solverConfig.lb = [kron(ones(N,1), xmin); kron(ones(N,1), umin); zeros(N*dimC, 1)];
solverConfig.ub = [kron(ones(N,1), xmax); kron(ones(N,1), umax); Inf*ones(N*dimC, 1)];
